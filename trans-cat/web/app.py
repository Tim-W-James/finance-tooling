...
def uploadFiles():
    ...
    if "Category" in asdd:
        df_y = pd.DataFrame(asdd, columns=['Category']).values.tolist()
        df_y_flat = list(dict.fromkeys( [subitem for item in df_y for subitem in item] ))
        y_existing_str = asdd["Category"]
        asdd.drop(["Category"], axis=1, inplace=True)
    else:
        df_y_flat = []
        y_existing_str = []
...
def save_model_create():
    ...
    df_pred.insert(len(df_pred.columns), "Category", feture_vec)
...
def train_model_predict():
    ...
    df_y = pd.DataFrame(df, columns=['Category']).values.tolist()
    df_y_flat = list(dict.fromkeys( [subitem for item in df_y for subitem in item] ))
